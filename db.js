...
var DbHelper = function (connectionURL) {
  ...
  this.createEvent = async function (event) {
    ...
    await client.query("INSERT INTO events (event_name, event_type, created_date, event_date, event_limit, event_address, username) VALUES  ($1,$2,$3,$4,$5,$6,$7)", [event.event_name, event.type, event.created_date, event.date, event.event_limit, event.address, event.username], function (err, result) {
      if (err) {
        console.log(err);
        return false;
      } else {
